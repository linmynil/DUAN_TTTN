...
const History: React.FC<PropsType> = (props) => {
    ...
    const handleSelect = (item: Item) => {
        const name = item.name_staff as string;
        const phone = item.phone_staff as string;
        const avatar = item.avatar_staff as string
    ...
    const fetchDoneData = async () => {
        try {
            const response = await axios.get("http://192.168.1.54:3000/report/getAllHistory");
    ...
    return role == 0 ? (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.switchButton}>
                ...
                <View>
                    {selectTab === 0 ? renderReports(dataReports) : renderReports(doneReports)}
                </View>
            </View>
